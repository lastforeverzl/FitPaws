...
class MonthTab extends React.Component {
  ...
  _calculate = (historyData, date) => {
    const monthData = historyData.filter(item =>
      item.creationDate.getFullYear() === date.getFullYear() &&
      item.creationDate.getMonth() === date.getMonth());
    this.setState({ data: monthData });
    if (monthData.length > 0) {
      const sum = (acc, cur) => acc + cur;
      const totalWalk = monthData.length;
      const totalMinutes = monthData.map(item => item.time).reduce(sum);
      const totalDistance = monthData.map(item => item.distance).reduce(sum);
