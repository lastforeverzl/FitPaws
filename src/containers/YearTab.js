...
class YearTab extends React.Component {
  ...
  _calculate = (historyData, date) => {
    ...
    if (yearData.length > 0) {
      ...
      const totalWalk = yearData.length;
      const totalMinutes = yearData.map(item => item.time).reduce(sum);
      const totalDistance = yearData.map(item => item.distance).reduce(sum);
